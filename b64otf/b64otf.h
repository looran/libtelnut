...
// awk cannot reliably print non-printable chars with %c, do it in shell, see below

#define B64_DECODER_ONELINER "a() { base$(( 2 ** 6 )) -d ; } ; b() { awk 'BEGIN { F=\"ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/\" ; while( getline < \"/dev/stdin\" ) { A = length( $0 ); for( B = 1; B <= A; ++B ) { C = index( F, substr( $0, B, 1 ) ); if( C-- ) { for( D = 0; D < 6; ++D ) { E = E*2+int( C/32 ); C = (C*2)%64; if( ++EDC == 8 ) { printf \"%x\\n\", E; EDC = 0; E = 0; } } } } } }' |while read a; do printf \"\\x$a\"; done } ; c() { A=0 ; B=0 ; C=0 ; while read -n1 D ; do E=$(printf %i \\\'\"$D\") ; if [ $E -eq 43 ] ; then F=62 ; elif [ $E -eq 47 ]; then F=63 ; elif [ $E -lt 48 ]; then continue ; elif [ $E -lt 58 ]; then F=$(( $E + 4 )) ; elif [ $E -eq 61 ]; then C=$(( $C + 1 )) ; F=0 ; elif [ $E -lt 65 ]; then continue ; elif [ $E -lt 91 ]; then F=$(( $E - 65 )) ; elif [ $E -lt 97 ]; then continue ; elif [ $E -lt 123 ]; then F=$(( $E - 71 )) ; else continue ; fi ; A=$(( $A + 1 )) ; B=$(( ($B << 6) | $F )) ; if [ $A -eq 4 ] ; then G=$(printf \"%x\" $(( ($B >> 16) & 0xFF ))) ; printf \"\\x$G\" ; if [ $C -lt 2 ]; then G=$(printf \"%x\" $(( ($B >> 8) & 0xFF ))) ; printf \"\\x$G\" ; fi ; if [ $C -eq 0 ]; then G=$(printf \"%x\" $(( $B & 0xFF ))) ; printf \"\\x$G\" ; fi ; A=0 ; B=0 ; C=0 ; fi ; done ; } ; [ X\"$(echo -n 'YQ==' |xbase$(( 2 ** 6 )) -d 2>/dev/null)\" == X\"a\" ] && a && exit ; [ X\"$(echo -n 'a b' |awk '{print $1}' 2>/dev/null)\" == X\"a\" ] && b && exit ; c ;"
